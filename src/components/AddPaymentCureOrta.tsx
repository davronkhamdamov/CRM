...
const AddPaymentCureOrta: React.FC<EditModalProps> = ({ data, setOpen }) => {
  ...
  const PayWithBalance: FormProps<CureFieldType>["onFinish"] = (e) => {
    setLoadingCnx(true);
    fetch(`${import.meta.env.VITE_APP_URL}/orto-cure/pay-balance/` + data.id, {
      method: "PUT",
      headers: {
  ...
  const PayWithCash: FormProps<CureFieldType>["onFinish"] = (e) => {
    setLoadingCnx(true);
    fetch(`${import.meta.env.VITE_APP_URL}/orto-cure/pay/` + data.id, {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-type": "application/json",
...
export default AddPaymentCureOrta;
